...
if (empty($myrow2['id']))
   {
    ...
    exit("Entrance to this page is allowed only for administrator! Go to <a href = 'http://liiissj2.beget.tech/table.php' >Main Page</a>");
   }
}
else {
...
<?php
    ...
    $login = 'administrator';
    $test = mysql_query("SELECT * FROM `users` WHERE `login` = '$login'",$connect) or die ( mysql_error() );
    $row = mysql_fetch_array($test);

    if (empty($row['login'])) {

        echo 'Неверные данные. <a href="index.php">Перейти на главную страницу.</a> ';
}
...
?>
    ...
    <div class="footer">
        ...
        <a href="table.php" class="register">Назад</a>
    </div>
